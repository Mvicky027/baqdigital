...
export function TuraDashboardStep({ onNavigate }: TuraDashboardStepProps) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans">
            ...
            <header className="bg-white shadow-sm sticky top-0 z-50">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <h1 className="text-violet-800 text-2xl font-bold tracking-tighter flex items-center gap-1">
                            EPS <span className="font-light">tura</span>
                            <div className="flex flex-col gap-0.5 ml-1 scale-75">
                                <div className="w-6 h-1 bg-violet-500 rounded-full rotate-[-15deg] translate-x-2"></div>
                                <div className="w-5 h-1 bg-violet-500 rounded-full rotate-[-15deg] translate-x-1"></div>
                                <div className="w-4 h-1 bg-violet-500 rounded-full rotate-[-15deg]"></div>
                            </div>
                        </h1>
                    </div>
                    ...
                    <button
                        ...
                        className="md:hidden p-2 text-violet-800 hover:bg-violet-50 rounded-full"
                    >
                        <ChevronDown className="w-6 h-6 rotate-90" />
                    </button>

                    <nav className="hidden md:flex items-center gap-6 text-sm text-gray-600 font-medium">
                        <a href="#" className="hover:text-violet-800">Inicio</a>

                        <div className="relative group">
                            <button
                                className="flex items-center gap-1 hover:text-violet-800"
                            ...
                            {isCitasOpen && (
                                <div
                                    className="absolute top-full left-0 w-80 bg-white shadow-lg rounded-b-lg border-t-2 border-violet-500 py-2 z-50"
                        ...
                        <a href="#" className="flex items-center gap-1 hover:text-violet-800">Pagos y reembolsos <ChevronDown className="w-4 h-4" /></a>
                        <a href="#" className="flex items-center gap-1 hover:text-violet-800">Afiliaciones y retiros <ChevronDown className="w-4 h-4" /></a>
                        <a href="#" className="flex items-center gap-1 hover:text-violet-800">Solicitudes y autorizaciones <ChevronDown className="w-4 h-4" /></a>
                        <a href="#" className="flex items-center gap-1 hover:text-violet-800">Certificados y consultas <ChevronDown className="w-4 h-4" /></a>
            ...
            {/* Blue Bar */}
            <div className="bg-violet-600 h-12 flex items-center justify-between px-4 md:px-8 text-white text-sm">
                <div className="font-bold">EPS</div>
                <div>Directorio médico PBS</div>
            </div>

            {/* Gray Bar */}
            <div className="bg-gray-200 h-8 flex items-center justify-between px-4 md:px-8 text-xs text-gray-600">
                <span>Inicio</span>
                <button
                    onClick={() => window.location.href = '/dashboard'}
                    className="flex items-center gap-1 hover:text-violet-800 font-medium md:hidden"
                >
                    Volver al Dashboard
                </button>
                <button
                    onClick={() => window.location.href = '/dashboard'}
                    className="hidden md:flex items-center gap-2 px-3 py-1 bg-white rounded-full text-violet-700 hover:bg-violet-50 font-medium transition-colors shadow-sm"
                >
                    Volver al Dashboard
                </button>
            </div>
            ...
            <main className="container mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
                ...
                <div className="flex-1">
                    <h2 className="text-violet-800 text-3xl font-bold mb-1">Servicios a un clic</h2>
                    <h3 className="text-gray-600 text-2xl mb-4">Hola, MARIA</h3>
                    <p className="text-gray-500 text-sm mb-2">Realiza fácilmente tus procesos con EPS TURA.</p>
                    <p className="text-gray-500 text-sm mb-8">¡Conoce aquí las soluciones virtuales que tenemos especialmente para ti!</p>

                    <h4 className="text-violet-800 font-bold mb-6">Los servicios más utilizados</h4>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        >
                            <Calendar className="w-8 h-8 text-violet-800" />
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-4">
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-3 items-start border-b border-gray-100 pb-3">
                                <User className="w-5 h-5 text-violet-800 mt-1" />
                                <div>
                                    <p className="text-xs text-gray-500 uppercase">MEDICO DE FAMILIA</p>
                                    <p className="text-xs font-bold text-gray-700">JUAN PEREZ (MÉDICO GENERAL)</p>
                                    <a href="#" className="text-violet-800 text-xs font-medium">Ver más</a>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3 items-start border-b border-gray-100 pb-3">
                                <Hospital className="w-5 h-5 text-violet-800 mt-1" />
                                <div>
